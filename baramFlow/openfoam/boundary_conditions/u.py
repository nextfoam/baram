...
class U(BoundaryCondition):
    ...
    def _constructBoundaryField(self):
        ...
        for bcid, name, type_ in self._region.boundaries:
            ...
            field[name] = {
                ...
                BoundaryType.OPEN_CHANNEL_INLET.value:  (lambda: self._constructVariableHeightFlowRateInletVelocity(self._db.retrieveValue(xpath + '/openChannelInlet/volumeFlowRate'))),
                BoundaryType.OPEN_CHANNEL_OUTLET.value: (lambda: self._constructOutletPhaseMeanVelocity(self._db.retrieveValue(xpath + '/openChannelOutlet/meanVelocity'))),
    ...
    def _constructFlowRateInletVelocity(self, xpath):
        spec = self._db.retrieveValue(xpath + '/flowRate/specification')
        if spec == FlowRateInletSpecification.VOLUME_FLOW_RATE.value:
            return {
                'type': 'flowRateInletVelocity',
                'volumetricFlowRate': self._db.retrieveValue(xpath + '/flowRate/volumeFlowRate')
            }
        elif spec == FlowRateInletSpecification.MASS_FLOW_RATE.value:
            return {
                'type': 'flowRateInletVelocity',
                'massFlowRate': self._db.retrieveValue(xpath + '/flowRate/massFlowRate'),
    ...
    def _constructRotatingWallVelocity(self, xpath):
        return {
            ...
            'omega': float(self._db.retrieveValue(xpath + '/speed')) * 2 * 3.141592 / 60,
        }

    def _constructVelocityInletU(self, xpath, name):
        spec = self._db.retrieveValue(xpath + '/velocityInlet/velocity/specification')
        if spec == VelocitySpecification.COMPONENT.value:
            profile = self._db.retrieveValue(xpath + '/velocityInlet/velocity/component/profile')
            if profile == VelocityProfile.CONSTANT.value:
                ...
                return self._constructTimeVaryingMappedFixedValue(
                    ...
                    Project.instance().fileDB().getFileContents(
                        self._db.retrieveValue(xpath + '/velocityInlet/velocity/component/spatialDistribution')))
            ...
            profile = self._db.retrieveValue(xpath + '/velocityInlet/velocity/magnitudeNormal/profile')
            if profile == VelocityProfile.CONSTANT.value:
                return self._constructSurfaceNormalFixedValue(
                    self._db.retrieveValue(xpath + '/velocityInlet/velocity/magnitudeNormal/constant'))
            elif profile == VelocityProfile.SPATIAL_DISTRIBUTION.value:
                return self._constructTimeVaryingMappedFixedValue(
                    self._region.rname, name, 'U',
                    Project.instance().fileDB().getFileContents(
                        self._db.retrieveValue(xpath + '/velocityInlet/velocity/magnitudeNormal/spatialDistribution')))
            elif profile == VelocityProfile.TEMPORAL_DISTRIBUTION.value:
