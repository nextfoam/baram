...
from .parallelogram_dialog_ui import Ui_ParallelogramDialog


class ParallelogramDialog(QDialog):
    def __init__(self, parent, line: Parallelogram, isNew=False):
        super().__init__(parent)

        self._ui = Ui_ParallelogramDialog()
        ...
        self._ui.point1Samples.setValidator(QIntValidator())
        self._ui.point2Samples.setValidator(QIntValidator())

        self._ui.name.setText(line.name)

        self._ui.originX.setText(line.originX)
        self._ui.originY.setText(line.originY)
        self._ui.originZ.setText(line.originZ)
        ...
        self._ui.point1Samples.setText(str(line.point1Samples))
        self._ui.point2Samples.setText(str(line.point2Samples))
    ...
    async def _okClicked(self):
        ...
        self._line.originX = self._ui.originX.text()
        self._line.originY = self._ui.originY.text()
        self._line.originZ = self._ui.originZ.text()
        ...
        self._line.point1Samples = int(self._ui.point1Samples.text())
        self._line.point2Samples = int(self._ui.point2Samples.text())
    ...
    async def _valid(self) -> bool:
        ...
        point1Samples = int(self._ui.point1Samples.text())
        point2Samples = int(self._ui.point2Samples.text())
        if point1Samples < 2 or point2Samples < 2:
            await AsyncMessageBox().critical(self, self.tr('Input Error'),
