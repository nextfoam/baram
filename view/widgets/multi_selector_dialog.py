...
class MultiSelectorDialog(QDialog):
    def __init__(self, parent, title, items, selectedItems):
        ...
        Args:
            ...
            selectedItems: List of display texts of selected items
        """
        super().__init__()
    ...
    def _connectSignalsSlots(self):
        ...
        self._ui.list.itemDoubleClicked.connect(self._add)
        self._ui.add.clicked.connect(self._add)
        self._ui.remove.clicked.connect(self._remove)
        self._ui.selectedList.itemDoubleClicked.connect(self._remove)
    ...
    def _add(self):
        for i in self._ui.list.selectedItems():
            i.setData(ListDataType.SELECTION_FLAG.value, True)
            i.setHidden(True)
            i.setSelected(False)
            item = QListWidgetItem(i.text())
            item.setData(Qt.UserRole, self._ui.list.row(i))
            self._ui.selectedList.addItem(item)
