...
class InitializationDB:
    ...
    def getTemperature(cls, rname: str) -> float:
        return float(
            coredb.CoreDB().getValue(f'.//regions/region[name="{rname}"]/initialization/initialValues/temperature'))

    @classmethod
    def getScaleOfVelocity(cls, rname: str) -> float:
        return float(
            coredb.CoreDB().getValue(f'.//regions/region[name="{rname}"]/initialization/initialValues/scaleOfVelocity'))

    @classmethod
    def getTurbulentViscosity(cls, rname: str) -> float:
        return float(
            coredb.CoreDB().getValue(f'.//regions/region[name="{rname}"]/initialization/initialValues/turbulentViscosity'))

    @classmethod
    def getTurbulentIntensity(cls, rname: str) -> float:
        return float(
            coredb.CoreDB().getValue(f'.//regions/region[name="{rname}"]/initialization/initialValues/turbulentIntensity')) / 100.0
